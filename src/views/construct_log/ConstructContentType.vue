    ...
    <div>
        <el-dialog title="班组新增" :visible.sync="dialogFormVisible" width="40%">
            <el-form :model="form" status-icon ref="form">
                <el-form-item label="类型名称" :label-width="formLabelWidth" prop="teamsName">
                    <el-input v-model="form.constructContentTypeName" autocomplete="off" style="width:83%"></el-input>
                </el-form-item>
        ...
        <el-table
            ...
            <el-table-column
                    label="操作">
                <template slot-scope="scope">
                    <el-button type="text" @click="updateConstructContentType(scope.row)">编辑</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updateConstructContentType(row){
                //编辑页面
            ...
            //提交
            submit(form){
                this.api.addConstructContentType(this.form).then(res =>{
                    this.initDatas();
                    this.dialogFormVisible = false;
            ...
            //删除选中
            deleteSelect(row){
                this.api.deleteConstructContentTypeList(this.multipleSelection).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            //删除按钮
            deleteSelects(row){
                this.api.deleteConstructContentTypeList([row]).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            initDatas(){
                this.api.getConstructContentTypeList({
                    'pageVo':{
