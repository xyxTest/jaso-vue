<template>
    <div>
        <el-dialog title="存在问题新增" :visible.sync="dialogFormVisible" width="40%">
            <el-form :model="form" status-icon ref="form">
                <el-form-item label="问题名称" :label-width="formLabelWidth" prop="existingProblemName">
                    <el-input v-model="form.existingProblemName" autocomplete="off" style="width:83%"></el-input>
                </el-form-item>
        ...
        <el-table
            ...
            <el-table-column
                    label="操作">
                <template slot-scope="scope">
                    <el-button type="text" @click="updateExistingProblem(scope.row)">编辑</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //提交
            submit(form){
                this.api.addExistingProblem(this.form).then(res =>{
                    this.initDatas();
                    this.dialogFormVisible = false;
            ...
            //删除选中
            deleteSelect(row){
                this.api.deleteExistingProblemList(this.multipleSelection).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            //删除按钮
            deleteSelects(row){
                this.api.deleteExistingProblemList([row]).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            initDatas(){
                this.api.getExistingProblemList({
                    'pageVo':{
