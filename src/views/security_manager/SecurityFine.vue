<template>
    <div>
        <el-dialog title="安全奖惩单新增" :visible.sync="dialogFormVisible" width="40%">
            <el-form :model="form" status-icon ref="form">
                <el-form-item label="奖惩事由" :label-width="formLabelWidth" prop="securityCheckId">
                    <el-select v-model="form.securityCheckId" placeholder="请选择事由" style="width: 180px; margin:10px 5px;  ">
                        <el-option
                                v-for="item in securityCheckList"
                                :key="item.securityCheckId"
                                :label="item.securityCheckName"
                                :value="item.securityCheckId">
                        </el-option>
                    </el-select>
                </el-form-item>
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <el-button type="text" @click="updateSecurityFine(scope.row)">编辑</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            getAllSecurityCheck(){
                ...
                securityManager.getSecurityCheckList({"projectId":projectId,"pageVo":pageParams}).then(res =>{
                    debugger
                    this.securityCheckList = res.data;
                }).catch(res =>{
            ...
            getAboutDepartmentList(){
                 let projectId = sessionStorage.getItem("projectId");
                securityManager.getAboutDepartmentList({"projectId":projectId}).then(res =>{
                    debugger
            ...
            submit(form){
                ...
                securityManager.addSecurityFine(this.form).then(res =>{
                    this.initDatas();
                    this.dialogFormVisible = false;
            ...
            //删除选中
            deleteSelect(row){
                securityManager.deleteSecurityFine(this.multipleSelection).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            //删除按钮
            deleteSelects(row){
                securityManager.deleteSecurityFine([row]).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            initDatas(){
               securityManager.getSecurityFine({
                    'pageVo':{
