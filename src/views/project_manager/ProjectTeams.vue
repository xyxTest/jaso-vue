    ...
    <div>
        <el-dialog title="班组新增" :visible.sync="dialogFormVisible" width="40%">
            <el-form :model="form" status-icon ref="form">
                <el-form-item label="班组名称" :label-width="formLabelWidth" prop="teamsName">
                    <el-input v-model="form.teamsName" autocomplete="off" style="width:83%"></el-input>
                </el-form-item>
                <el-form-item label="班组人数" :label-width="formLabelWidth" prop="peopleNum">
                    <el-input v-model="form.peopleNum" autocomplete="off" style="width:83%"></el-input>
                </el-form-item>
            </el-form>
    ...
    export default {
        ...
        methods: {
            ...
            submit(form){
                ...
                this.api.addProjectTeams(this.form).then(res =>{
                    this.initDatas();
                    this.dialogFormVisible = false;
            ...
            //删除选中
            deleteSelect(row){
                this.api.deleteProjectTeams(this.multipleSelection).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            //删除按钮
            deleteSelects(row){
                this.api.deleteProjectTeams([row]).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
