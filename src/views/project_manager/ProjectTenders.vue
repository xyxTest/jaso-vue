<template>
    <div>
        <el-dialog title="标段新增" :visible.sync="dialogFormVisible" width="40%">
            <el-form :model="form" status-icon ref="form">
                <el-form-item label="标段名称" :label-width="formLabelWidth" prop="tendersName">
                    <el-input v-model="form.tendersName" autocomplete="off" style="width:83%"></el-input>
                </el-form-item>
            </el-form>
        ...
        <el-table
            ...
            <el-table-column
                    label="操作">
                <template slot-scope="scope">
                    <el-button type="text" @click="updateTenders(scope.row)">编辑</el-button>
    ...
    export default {
        ...
        methods: {
            ...
            submit(form){
                ...
                this.api.addProjectTenders(this.form).then(res =>{
                    this.initDatas();
                    this.dialogFormVisible = false;
            ...
            //删除选中
            deleteSelect(row){
                this.api.deleteProjectTenders(this.multipleSelection).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            //删除按钮
            deleteSelects(row){
                this.api.deleteProjectTenders([row]).then( res =>{
                    this.$message.success(res.message);
                    this.initDatas();
                }).catch(res =>{
                    this.$message.error(res.message);
            ...
            initDatas(){
                this.api.getProjectTendersList({
                    'pageVo':{
